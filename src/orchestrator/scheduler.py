...
class TaskScheduler:
        def __init__(self, clock: Callable[[], float] = time.monotonic):
                    self._clock = clocklf._last_heartbeat: Dict[str, float] = {}
        self._audit: List[Dict] = []def dequeue(self, queue: str = "default", timeout: float = 1.0) -> Optional[Dict]:
                    now = self._clock()
        ...
        if queue in self._queues and len(self._queues[queue]) > 0:
            ...
            if task:
                                self._in_flight[task["id"]] = tasksk:
